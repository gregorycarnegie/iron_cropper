...
pub fn perform_detection(
    ...
    let image = {
        let _guard = timing_guard("fcs_gui::load_image", log::Level::Debug);
        Arc::new(
            load_image(&path)
    ...
    let detection_output = {
        let _guard = timing_guard("fcs_gui::detect_image", log::Level::Debug);
        detector
            .detect_image(&image)
    ...
    let detections_with_quality: Vec<DetectionWithQuality> = {
        let _guard = timing_guard("fcs_gui::estimate_sharpness_batch", log::Level::Debug);
        detection_output
            .detections
            .into_iter()
            .map(|detection| {
...
pub fn create_thumbnails(
    ...
    let _guard = timing_guard("fcs_gui::create_thumbnails", log::Level::Debug);
