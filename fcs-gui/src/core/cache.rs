...
use fcs_utils::gpu::{GpuStatusIndicator, red_eye::RedEye};
...
use fcs_utils::{
    BatchCropRequest, GpuBatchCropper, WgpuEnhancer,
...
pub fn calculate_eyes_relative_to_crop(
    landmarks: &[fcs_core::Landmark; 5],
    crop_region: &fcs_core::CropRegion,
    img_width: u32,
    img_height: u32,
    target_width: u32,
    target_height: u32,
...
pub fn calculate_eyes_absolute(landmarks: &[fcs_core::Landmark; 5]) -> Vec<RedEye> {
    // Landmarks 0 and 1 are eyes
    let mut eye_list = Vec::with_capacity(2);
    for i in 0..2 {
...
pub fn ensure_crop_preview_entry(
    ...
    composite_with_fill_color(&mut rgba, crop_settings.fill_color);
...
fn pack_rgba_color(color: fcs_utils::color::RgbaColor) -> u32 {
    ((color.alpha as u32) << 24)
        | ((color.red as u32) << 16)
        | ((color.green as u32) << 8)
        | (color.blue as u32)
