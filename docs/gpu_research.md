...
- **Unit tests**:
  ...
  - GPU↔CPU gating: `cargo test -p fcs-core gpu_inference_matches_cpu_baseline -- --nocapture` exercises `fcs-core/tests/gpu_cpu_parity.rs`, running mixed fixtures (single, group, occluded, negative) through real GPU preprocess/inference stacks, enforcing 1e-3 score and 5 px bbox/landmark tolerances and emitting max deltas.
